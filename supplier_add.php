...
/**
 * =====================================================
 * 文件名：supplier_add.php
 * 功能：新增供应商
 * 描述：添加供应商到供应商库，支持基本信息、财务信息等
 * 版本：1.0
 * 更新日期：2025-10-22
 * =====================================================
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // 生成供应商编号
        $supplier_code = 'S' . date('Ymd') . sprintf('%04d', rand(1000, 9999));
        
        // 插入供应商
        $stmt = $conn->prepare("
            INSERT INTO suppliers 
            (supplier_code, company_name, contact_person, contact_phone, contact_email, 
             address, tax_number, bank_name, bank_account, payment_terms, credit_limit, 
        ...
        if (!$stmt->execute()) {
            throw new Exception('供应商添加失败：' . $stmt->error);
        }
...
<body>
    ...
    <main class="main-content">
        <div class="page-header">
            ...
            <div class="btn-group">
                ...
                <button type="button" class="btn btn-success" onclick="saveSupplier()">💾 保存供应商</button>
            </div>
        </div>

        <div class="alert alert-info">
            💡 提示：标记为必填项（*）的字段必须填写。供应商编号将自动生成。
        </div>
        ...
        <div class="form-card">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="company_name" class="form-control" placeholder="例如：XX科技有限公司">
                </div>

                <div class="form-group">
                    <label class="required">联系人</label>
                    <input type="text" id="contact_person" class="form-control" placeholder="例如：张经理">
                </div>

                <div class="form-group">
                    <label class="required">联系电话</label>
                    <input type="text" id="contact_phone" class="form-control" placeholder="例如：13800138000">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>联系邮箱</label>
                    <input type="email" id="contact_email" class="form-control" placeholder="例如：supplier@example.com">
                </div>

                <div class="form-group full-width">
                    <label>公司地址</label>
                    <input type="text" id="address" class="form-control" placeholder="例如：北京市朝阳区XX路XX号">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>税号</label>
                    <input type="text" id="tax_number" class="form-control" placeholder="纳税人识别号">
                    <span class="form-help">用于开具发票</span>
                </div>

                <div class="form-group">
                    <label>开户行</label>
                    <input type="text" id="bank_name" class="form-control" placeholder="例如：中国工商银行XX支行">
                </div>

                <div class="form-group">
                    <label>银行账号</label>
                    <input type="text" id="bank_account" class="form-control" placeholder="银行账号">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>账期（天）</label>
                    <input type="number" id="payment_terms" class="form-control" value="30" min="0" placeholder="付款账期">
                    <span class="form-help">约定的付款天数，0表示现结</span>
                </div>

                <div class="form-group">
                    <label>信用额度（元）</label>
                    <input type="number" id="credit_limit" class="form-control" value="0" min="0" step="0.01" placeholder="信用额度">
        ...
        <div class="form-card">
            ...
            <div class="form-row">
                <div class="form-group full-width">
                    <label>备注说明</label>
                    <textarea id="description" class="form-control" placeholder="供应商的其他说明信息"></textarea>
                </div>
            </div>
    ...
    <script>
        ...
        async function saveSupplier() {
            ...
            try {
                const response = await fetch('supplier_add.php', {
                    method: 'POST',
                    headers: {
